...
public class PageTest {
  ...
  void holdsExtractors() {
    ...
    RegexPattern sqft = new RegexPattern("SqFt").add("\"area\":").add("Square_Feet", "[^'\"]*").add(",");
    RegexPattern bath = new RegexPattern("Bath").add("\"baths\":").add("Bathrooms", "[^'\"]*").add(",");
    RegexPattern beds = new RegexPattern("Beds").add("\"beds\":").add("Bedrooms", "[^'\"]*").add(",");
    RegexPattern prop = new RegexPattern("Prop Type").add("\"PropertyType\", \"").add("Property_Type", "[^'\"]*").add("\"],");
    RegexPattern year = new RegexPattern("Year").add(",\"year\":").add("Year_Built", "[\\d,]+").add(",");
    RegexPattern cost = new RegexPattern("Price").add("\"typeAndPrice\":\"For Rent: $").add("Price", ".*?").add("/mo\"");
    RegexPattern cp1  = new RegexPattern("Phone1").add("Call: ").add("Contact_Phone1", "[^<>]*?").add("<\\/span>\\s*<\\/div>");
  ...
  void runPage() {
    ...
    CrawlerConf conf = new CrawlerConf(SeleniumDriverType.CHROME, false, false, false, false, null, "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_13_3) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/63.0.3239.132 Safari/537.36", -1);
